...
/**
 * Interfaz exclusiva del Desarrollador.
 * Muestra los proyectos en los que participa, sus tareas asignadas,
 * el progreso general del proyecto y el feedback del gerente.
 */
public class DeveloperDashboard extends JPanel {
    ...
    private DeveloperProjectPanel panelDetalle;

    public DeveloperDashboard(ProjectService projectService, Runnable alCerrarSesion) {
    ...
    private JPanel crearCabeceraSidebar() {
        ...
        JLabel usuario = new JLabel(nombre + " · Desarrollador");
    ...
    private JScrollPane crearScrollLista() {
        ...
        listaProyectos.setCellRenderer((list, value, index, isSelected, cellHasFocus) -> {
            ...
            // Cuenta cuántas tareas tiene asignadas este desarrollador
            String devId = SessionManager.getInstance().getUsuarioActivo().getId();
            long misTareas = value.getTareas().stream()
                    .filter(t -> t.getAsignadoAId().equals(devId)).count();

            JLabel info = new JLabel(value.getEstadoNombre() + "  |  " + misTareas + " tareas mías");
            info.setFont(AppTheme.FUENTE_PEQUEÑA);
            info.setForeground(AppTheme.AMARILLO_AVISO);

            celda.add(nombre, BorderLayout.NORTH);
            celda.add(info,   BorderLayout.SOUTH);
            return celda;
        });
    ...
    private JPanel crearPanelVacio() {
        ...
        JLabel msg = new JLabel("Selecciona un proyecto para ver tus tareas");
    ...
    private void seleccionarProyecto() {
        ...
        panelDetalle = new DeveloperProjectPanel(proyecto, this::refrescarLista);
    ...
    private void refrescarLista() {
        ...
        modeloLista.clear();
        // El desarrollador solo ve proyectos donde tiene al menos una tarea asignada
        for (Project p : projectService.obtenerTodos()) {
            boolean tieneParticipacion = p.getTareas().stream()
                    .anyMatch(t -> t.getAsignadoAId().equals(devId));
            if (tieneParticipacion) modeloLista.addElement(p);
        }
